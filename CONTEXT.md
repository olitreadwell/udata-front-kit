...
> refreshed 2026-09-25 | upstream default: main @ 3f24a1a5b9641da52133ce6bbc71bf1fc1ef956c
...
- `2026-09-24` self-found a11y gap — outcome: pr-opened (https://github.com/olitreadwell/udata-front-kit/pull/9, branch fix/keyboard-accessible-home-tiles) — SubSectionTiles.vue home "tiles" section cards (used on defis + hackathon home pages) rendered as `<div @click>` with a JS `window.location.href` handler: no keyboard focus, no Enter activation, no link role, no focus-visible outline. Replaced with a real `<a :href>` link (native focus, Enter, focus-visible, open-in-new-tab), removed the goToPage handler. Sibling section components already use a link (SubSectionCards) or a button (SubSectionButtons), so tiles now match. Added regression test src/components/sections/**tests**/SubSectionTiles.test.ts (fails on the old div, passes on the link). Local verify: vitest 74 pass (was 70), lint, type-check:app+cypress, format:check all green. Fork CI fully green 10/10 (Lint+type-check, Unit, 8× e2e incl defis + hackathon). mergeable_state clean. No AI in body/commit. Deduped: no upstream issue/PR claimed this gap.

- `2026-09-25` self-found trivial cleanup (French typos) — outcome: pr-opened (https://github.com/olitreadwell/udata-front-kit/pull/10, branch fix/trivial-cleanup-2026-09-25) — 9 genuine meaning-preserving French typo fixes in 6 files: accesssibilité→accessibilité (configs/accessibilite/config.yaml meta_description); succinte→succincte (hackathon/pages/pendant-le-hackathon.md); reférencées→référencées (simplifions/pages/niveaux-simplification.md); erronnée→erronée ×2 + plubique→publique (ArticleAnticiperParcoursUsager.vue); administrtation→administration + lorqu'elles→lorsqu'elles (ArticlePrerequisEtapesIntegrationAPI.vue, prettier reflow); interministerielle→interministérielle (ArticleGuideBasePetitesCollectivites.vue). Local verify: type-check, lint --max-warnings 0, format:check, vitest 70 pass, simplifions build all green. No overlap with open PRs #8/#9 or merged #1375. Dedupe: French dialect honored, no variant normalization.

- `2026-09-25` self-found bug-fix gap — outcome: pr-opened (https://github.com/olitreadwell/udata-front-kit/pull/11, branch fix/legacy-factor-search-crash) — `useFactorsFilter` in src/utils/topicGroups.ts crashed on legacy factors with no `title` (`factor.title.toLowerCase()` threw TypeError) though the file already documents "some legacy factors have no title, fallback to '' to avoid errors" in `groupedFactors`. Added the same `(factor.title || '')` guard + regression test src/utils/__tests__/topicGroups.test.ts (fails on old code, passes with guard). Local verify: vitest 79 pass (was 70), type-check, lint --max-warnings 0, format:check all green. Dedupe: gh search issues+prs (topicGroups/useFactorsFilter/legacy/title) open+closed all empty; no overlap with open PRs #8/#9/#10 or merged #1375. Branch off fork main (in sync with upstream @3f24a1a5), NON-draft, base=fork main. 
